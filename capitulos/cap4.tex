...
Para determinação de uma equação geral foram considerados os dados de todos os indivíduos em um grande conjunto. Desconsideramos os grupos populacionais e utilizamos os dados de todos os seis grupos como entrada de um método de regressão multilinear. A amostra populacional randômica é de n=200 (número de indivíduos em cada grupo), considerando os 6 grupos tem-se então uma população para esta abordagem de n=1200. A influência de todas as variáveis independentes foi testada em relação à \acrshort{DEE} no modelo de regressão multilinear. Somente o \acrshort{IMC} demonstrou ter significância estatística (p<0,05) para este modelo. A equação de regressão \ref{equ:equacaoGenerica} representa este modelo. 
...
Continuar depois da equação geral
================================== 
